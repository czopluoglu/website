...
#2014 Il Genel Meclis Uyeligi

secimId        = 36039  # 2014 mahalli idare genel secim
secimturu      = 4      # il genel meclis uyeligi

buyuksehir.kod <- fromJSON("https://sonuc.ysk.gov.tr/api/getIlList?secimId=36039&secimTuru=4&sandikTuru=-1&yurtIciDisi=1")
...
parti.kod <- jsonlite::fromJSON("https://sonuc.ysk.gov.tr/api/getSandikSecimSonucBaslikList?secimId=36039&secimCevresiId=0&ilId=0&bagimsiz=0&secimTuru=4&yurtIciDisi=1")
...
colnames(sandik.sonuc)[8:36] <- paste0(colnames(sandik.sonuc)[8:36],"_ILBLMeclis2014")

write.csv(sandik.sonuc,"ILBLMeclis2014.csv",row.names=FALSE)
