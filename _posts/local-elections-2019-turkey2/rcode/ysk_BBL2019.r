...
#2019 Buyuksehir Belediye

secimId        = 49302  # 2019 mahalli idare genel secim
secimturu      = 6      # buyuksehir belediye
...
buyuksehir.kod$secimCevresiID <- c(272630,272631,272632,272633,272634,272635,272636,272637,
                                   272638,272639,272640,272641,
                                   272643,272644,272650,272645,272646,272647,272648,272649,
                                   272651,272642,272652,272653,272654,272655,272658,
                                   272656,272657,272659)
...
parti.kod <- jsonlite::fromJSON("https://sonuc.ysk.gov.tr/api/getSandikSecimSonucBaslikList?secimId=49302&secimCevresiId=0&ilId=0&bagimsiz=0&secimTuru=6&yurtIciDisi=1")
...
colnames(sandik.sonuc)[8:26] <- paste0(colnames(sandik.sonuc)[8:26],"_BBL2019")

write.csv(sandik.sonuc,"BBL2019.csv",row.names=FALSE)
