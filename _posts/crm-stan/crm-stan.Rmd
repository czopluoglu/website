...
The ORF dataset from MPS was the only reason I was interested in CRM at all. During my second year in the graduate school, I had a chance to complete a semester-long internship in MPS. The task I was assigned was to use IRT for scaling an ORF dataset they had. The dataset had  three variables representing the number of words read correctly per minute for three different reading passages. There were 2594 students. Below is a little information about the dataset.  

```{r echo = FALSE, eval=TRUE}

require(foreign)

d <- read.spss("C:/Users/Dr Zopluoglu/Box/Completed_Research/Internship/data/old data/original data/ORA.spring.2008.without ID.sav",
          to.data.frame=TRUE)
...
At the time, I had taken an IRT course and knew a little bit about dichotomous and polytomous IRT models. It was a challenge as the measurement outcomes were kind of continuous (they are simply count data). As I didn't know much about CRM at that time and how to deal with this, I created discrete categories for each variable such that Level 1 (0-50 words), Level 2(51-100 words), Level 3 (101-150 words), Level 4 (151-200 words), and Level 5 (201-250 words). Then, I fit polytomous IRT models and did usual IRT modeling based on the best fitting model. It was fine to complete the internship, but I was not satisfied as there was so much information thrown away by discretizing. That's how I ended up exploring and discovering CRM and started working on it. I first briefly introduce the model before demonstrating how to use it for ORF data.
...
where $k_j$ is the maximum possible score for the item. The parameter $a$ and $b$ have a very similar interpretation as in the 2PL IRT model. The $a$ parameter controls the steepness and the $b$ parameter controls the location of the operating characteristic curves. There is an extra $\alpha$ parameter which is called "scaling parameter", and it controls the distance among the curves. 

Let's create some plots to see it better. Suppose there is an item with 10 categories and this item has the following parameters: $a=1$, $b=0$, and $\alpha=1$.This is how the operating characteristic curves look like. Note that there are 9 lines in the plot indicating $P(X\geq 1)$, $P(X\geq 2)$, $P(X\geq 3)$, ..., $P(X\geq 9)$.
...
Now, let's keep everything same but increase the value of the a parameter to 3, and we will see how steepness changes.
...
Or, we can keep everything same but change the value of the $b$ parameter to -2 and we will see the curves shift to left.
...
Lastly, let's change the value of the $\alpha$ parameter to 4 and we will see how the distance among curves changes.
...
There are a number of ways we can fit CRM. I will focus here fitting the model using Stan, but I also provide other two ways at the very end as supplemental material. Before providing the Stan model syntax, I should first reformat data as this is the only way to run Stan with missing data. Although I don't have missing data in the ORF dataset used for this blog post, I force myself to use this procedure to get used to it.
...
# ad an arbitrary individual id variable
...
Below is the Stan model syntax for fitting CRM. This follows the guidelines in the Stan manual and imposes weakly-informative
...
Below code fits the model with 4 chains and 50,000 iterations for each chain. By default, first 50% of the iterations are used as burn-in and the remaining 50% of the iterations used for inference.
...
load("C:/Users/Dr Zopluoglu/Box/Blogs/crm.Rdata")
require(rstan)
require(psych)
```
...
It took about 4 hours to fit the model with the above settings with processor "Intel(R) Core(TM) i7-3840QM CPU @2.80GHz". I used only one core although this processor has 4 cores. Note that I could use all four cores and the estimation time would be reduced to 1 hour, However, my R kept crushing when I tried to do it for some reason I don't know, That might be related to my computer settings.
...
It seems that we have convergence and all chains yield consistant results as all $\hat{R}$ values are less than 1.01, Here is the posterior distributions for all these parameters. They all look fine.
...
As expected, the estimates have a mean of 0 and standard deviation of .99. 
...
For instance, the model provides an estimate of 3.11 for a student who had read 181 words, 224 words, and 220 words and an estimate of -2.28 for a student who had read 12 words, 6 words, and 8 words in the three reading passages, respectively. We can check how the estimated theta is related to the scores from each reading passage for all examinees.
...
I don't think there has been any work that seriously consider how to assess model fit for CRM. So, this certainly needs more work to be published in peer-review journal. However, we can try to borrow some ideas from other well-known IRT models.

The first thing I would like to look is a very simple idea of comparing model predictions to observed outcomes. Treating the item and person parameter estimates as known, we can calculate a predicted score on each item for each individual as well as a standardized residual.
...
There are some individuals with extreme residuals for each item; however, there doesn't seem to be any systematic pattern between residuals and latent variable.
...
Here, we can see there are 12 individuals with observations yielding extreme outliers. A quick examination of their score pattern reveals that these are typically individuals that scored really low for one or two reading passages while scoring relatively much higher in other reading passage. So, for instance, individual 919 read 20 and 18 words for Reading Passage 1 and 3 while reading 221 words for Reading Passage 2. This is extremely unusual. This may be due to data coding error, scoring error, low effor or motivation for some reason, etc. It might be a good idea to flag these individuals with inconsistent (or unreliable) scores, remove them from the dataset, and re-fit the model without these individuals.

After removing the outliers, the standardized residuals follow an approximately normal distributions.
...
Once you estimate the parameters from linear factor model, then you can transform them back to the IRT metric. 

The unstandardized factor loadings (0.824, 1.008, and 0.996) correspond the $\alpha$ parameters in CRM. You can check back the estimates from Stan, and you will see that they are identical. 
...
Finally, if you divide the intercepts by the the unstandardized factor loadings and then multiply the result by -1, that should give you the difficulty parameters of CRM ($b$).
...
This approach is fully described in two papers, [Wang and Zeng (1998)](https://journals.sagepub.com/doi/abs/10.1177/014662169802200402) and [Shojima(2005)](https://www.jstage.jst.go.jp/article/etr/28/1-2/28_KJ00003899231/_pdf/-char/ja). Wang and Zeng (1998) is a classical implementation of EM algorithm. However, Shojima(2005) is a really interesting paper as he actually derives the closed form equation for the marginal likelihood, so there is no integration needed for the E-step. When I was first interested in this model, I created an R package, [EstCRM](https://cran.r-project.org/web/packages/EstCRM/index.html), that implements both approaches. Below is a code using this package to fit the model.
